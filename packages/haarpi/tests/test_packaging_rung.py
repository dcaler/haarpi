...
def test_template_task_id_finds_the_pending_fetch_task():
    client = _FakeClient([{"id": 5, "project_id": 7, "title": "paper css2026 template 1"},
                          {"id": 6, "project_id": 7, "title": "paper css2026 outline 1"}])
    assert planner._template_task_id(client, _m(), "css2026") == 5


def test_no_template_task_means_no_dependency():
    client = _FakeClient([{"id": 6, "project_id": 7, "title": "paper css2026 outline 1"}])
...
def test_packaging_queues_a_runner_then_a_human_review():
    ...
    assert pkg["title"] == "paper css2026 package 2"           # one past the template's cycle 1
    ...
    assert review["title"] == "paper css2026 submission 2"
...
def test_packaging_without_a_template_task_has_no_dependency():
    ...
    assert pkg["title"] == "paper css2026 package 1"           # nothing prior -> cycle 1
